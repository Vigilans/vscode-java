...
class SemanticTokensProvider implements vscode.SemanticTokensProvider {
    ...
    /**
     * When received, add a `new tokens` opreation to versioned edit.
     */
    ...
    /**
     * When received, add a `shift` opreation to versioned edit.
     */
    ...
    /**
     * Convert tokens from language server to intermediate representation.
     */
    private decodeTokens(lines: SemanticHighlightingInformation[]): SemanticTokens {
        const tokens: SemanticTokens = new Map();
    ...
    /**
     * Convert tokens of intermediate representation to bytearray or edits that vscode receives.
     */
    ...
    /**
     * Pop an apply edits in the queue one by one.
     */
    private flushEdits(document: SemanticDocument, targetVersion: number) {
        while (document.currentVersion < targetVersion) {
            ...
            document.semanticTokens = this.applyEdit(document.semanticTokens, newEdit);
    ...
    /**
     * Pop and merge needed edits, then apply the merged edit. Suitable for generating vscode.SemanticTokensEdit.
     */
    private flushEdit2(document: SemanticDocument, targetVersion: number) {
    ...
    private applyEdit(tokens: SemanticTokens, edit: SemanticEdit): SemanticTokens {
        const newTokens: SemanticTokens = new Map();
